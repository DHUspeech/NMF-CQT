...
disp('denoising Female...')
    ...
    y0=y0(:,1);
     ...
     path=strcat('Denoised_male_speech/','m5/');
...
% save('mosM.mat','MOSs');
% save('stoM.mat','stoiS');
save('mosF_maleS_m5L.mat','MOSf');
save('stoF_maleS_m5L.mat','stoiF');
% 在计算 PESQ 分数后
pesq_results = [a; b; c]; % 将 PESQ 分数存储到一个向量中，方便保存
save('pesq_maleS_m5L.mat', 'pesq_results'); % 将 PESQ 结果保存到名为 'pesq_results.mat' 的文件中
