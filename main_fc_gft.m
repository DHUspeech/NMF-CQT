...
Ms1=zeros(24,1);Mf1=zeros(24,1);
Ts1=zeros(24,1);Tf1=zeros(24,1);
for tt=1:43
...
disp('denoising Female...')
    ...
    y0=y0(:,1);
     y1=resample(y0,fs,f);
     y=add_noisem(y1,['Noise/' NoiseData(i,1).name],snr(:,ss),fs);    
     [y4,y5,y6,y7]=Denoising2(y,Wfg1,Wfg2,Wfg3,W12,W22,W32,H12,H22,H32,all_r,fs,B,qz,alg,iter_num2,number_basesF,X);
     yr1=resample(y0,ff,f);
     yr4=resample(y4,ff,fs);
     yr5=resample(y5,ff,fs);
     yr6=resample(y6,ff,fs);
     yr7=resample(y7,ff,fs);
     a=pesq(yr1, yr4, ff);
     b=pesq(yr1, yr5, ff);
     c=pesq(yr1,yr6,ff);
     d=pesq(yr1,yr7,ff);
% %      Mf(:,ss)=[a,b]';
     VM=[a;b;c;d];Mf=[Mf;VM];
     [d1,d2,d3,d4]=Nstoi1(y1,y4,y5,y6,y7,fs);
%      d3=d2-d1;
%      Tf(:,ss)=[d1,d2]';
      VF=[d1;d2;d3;d4];Tf=[Tf;VF];
end
%      Ms1=Ms1+Ms;   
     Mf1=Mf1+Mf;
%      Ts1=Ts1+Ts;   
...
% save('mosM.mat','MOSs');
% save('stoM.mat','stoiS');
save('mosF-cf.mat','MOSf');
save('stoF-cf.mat','stoiF');
