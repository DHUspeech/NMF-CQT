...
% Sg3=[];
% Sg1L=[];
data_CQT=load('cqt_D.mat');
X_CQT=data_CQT.D;
data_FFT=load('fft_D.mat');
X_FFT=data_FFT.D;

L_CQT = diag(diag(X_CQT))-X_CQT;
[U0,S,V]=eig(L_CQT);
% [U0,S,V]=eig(X_CQT);
% [U,S,V]=eig(X);
X_CQT=U0';

L_FFT = diag(diag(X_FFT))-X_FFT;
[U,S,V]=eig(L_FFT);
% [U,S,V]=eig(X_FFT);
...
%     rr=size(absSp2,1);
%     all_r=floor(rr/2)+1;
%     absSp2=absSp2(1:all_r,:);
% %     f=gft(y,X);% gft
% %     absSp3=abs(f);
%     absSp1=abs(Xcq.c);
%     [~,absSp1]=gft1(absSp1,X_CQT);
%     [~,absSp3]=gft1(absSp2,X_FFT);
...
for i=1:GspeakerNum
    ...
    all_r=floor(rr/2)+1;
    absSp2=absSp2(1:all_r,:);
    phaseSp2=phaseSp(1:all_r,:);
    sp2=absSp2.*phaseSp2;
%     f=gft(y,X);% gft
%     absSp3=abs(f);
    sp1=Xcq.c;
    [~,abs_sp1]=gft1(sp1,X_CQT);
    [~,abs_sp3]=gft1(sp2,X_FFT);
    Fg1=[Fg1,abs_sp1];
    Fg2=[Fg2,absSp2];
    Fg3=[Fg3,abs_sp3];
...
%  save Wfg1 Hfg1 Wfg2 Hfg2 Wfg3 Hfg3
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%-Noise-%%%%%%%%%%%%%
...
disp('denoising Female...')
    ...
    y0=y0(:,1);
     y1=resample(y0,fs,f);
     y=add_noisem(y1,['Noise/' NoiseData(i,1).name],snr(:,ss),fs);    
     [y4,y5,y6]=Denoising1(y,Wfg1,Wfg2,Wfg3,W12,W22,W32,H12,H22,H32,all_r,fs,B,qz,alg,iter_num2,number_basesF,X_CQT,X_FFT);
     ...
     cqt_name=strcat('cqt_GFT',str_ss,noise_hz,'_',GTspeakerData(tt,1).name);
     gft_name=strcat('fft_GFT',str_ss,noise_hz,'_',GTspeakerData(tt,1).name);
     path=strcat('Denoised_female_speech/','join/');
...
% save('mosM.mat','MOSs');
% save('stoM.mat','stoiS');
save('mosF_m1Ljoin.mat','MOSf');
save('stoF_m1Ljoin.mat','stoiF');