...
% BspeakerData(1:2) =[];
% BspeakerNum=length(BspeakerData);%speakerNum:浜烘暟锛?
% Sg1=[];
% Sg2=[];
% Sg3=[];
% Sg1L=[];
...
GspeakerData(1:2) = [];
GspeakerNum=length(GspeakerData);%speakerNum:浜烘暟锛?
Fg1=[];
Fg2=[];
Fg3=[];
Fg1L=[];
...
% % [Wg1,Hg1,Wg2,Hg2,Wg3,Hg3]=sparse_NMF(Sg1,Sg2,Sg3,number_basesS,iter_num,alg); %% 濡傛灉瑕佺敤NMF鏉ユ洿鏂扮煩闃靛氨閫夋嫨NMF杩欎釜鍑芥暟
...
BTspeakerData(1:2) =[];
BTspeakerNum=length(BTspeakerData);%speakerNum:浜烘暟锛?

% waveDir='Female_speech524_test/';
GTspeakerData = dir(waveDir);
GTspeakerData(1:2) = [];
GTspeakerNum=length(GTspeakerData);%speakerNum:浜烘暟锛?

disp('training noise...')
waveDir_test='Noise/';
NoiseData = dir(waveDir_test);
NoiseData(1:2) = [];
NoiseNum=length(NoiseData);%speakerNum:浜烘暟锛?
...
disp('denoising Female...')
    ...
    y0=y0(:,1);
     ...
     path=strcat('Denoised_male_speech/','m3/');
...
% save('mosM.mat','MOSs');
% save('stoM.mat','stoiS');
save('mosF_maleS_m3L.mat','MOSf');
save('stoF_maleS_m3L.mat','stoiF');
% 在计算 PESQ 分数后
pesq_results = [a; b; c]; % 将 PESQ 分数存储到一个向量中，方便保存
save('pesq_maleS_m3L.mat', 'pesq_results'); % 将 PESQ 结果保存到名为 'pesq_results.mat' 的文件中
